...
Para cada fecha que presentaba vacíos térmicos prolongados en la base objetiva, se transfirió directamente el registro diario de temperatura media depurada ($T_{\text{med\_imputada}}$) correspondiente a su vecina más cercana en ese mismo día. Este procedimiento de interpolación espacial permitió recuperar con precisión climatológica un total de 1,114 observaciones diarias. Al término de esta tercera capa jerárquica de imputación, se logró una tasa de completitud global en las series de temperatura media diaria del 99.99\%, registrando únicamente 9 datos nulos residuales en toda la base de datos de panel maestro para las 18 bases a lo largo de los 24 años de estudio.
\end{enumerate}
...
Dado que el balance hídrico y el cálculo del SPEI se analizan a nivel mensual, el panel diario depurado de 18 bases fue agregado a una frecuencia mensual de 288 meses consecutivos por base (5,184 observaciones mensuales potenciales). 
...
Para cualquier escenario geográfico y estacional $s$, la tendencia neta mensual ($\beta_{\text{neta}, s}$) se define formalmente como:
\begin{equation}
    \beta_{\text{neta}, s} = \mathbf{w}_s' \boldsymbol{\hat{\beta}}
\label{eq:combinacion_lineal}
\end{equation}
Donde $\boldsymbol{\hat{\beta}}$ representa el vector de coeficientes estimados de dimensión $16 \times 1$, y $\mathbf{w}_s$ es un vector de pesos de dimensión $16 \times 1$ que selecciona los parámetros que interactúan con la variable temporal $T_t$ en el escenario $s$.
