...
\begin{enumerate}
    ...
    \item \textit{Homocedasticidad y no autocorrelación del error:} Supone que los errores idiosincráticos presentan una varianza constante e independiente a través de las observaciones temporales para una misma entidad. Debido a que las series climatológicas suelen exhibir autocorrelación y heterocedasticidad, este estudio aborda preventivamente dicha potencial desviación mediante la estimación de errores estándar robustos agrupados (clustered robust standard errors) a nivel de estación meteorológica. Esta técnica estadístico-analítica parametriza las matrices de covarianza para tolerar patrones arbitrarios de correlación y heterocedasticidad dentro de cada clúster individual, derivando p-valores y límites de confianza robustos para la inferencia de tendencias.
    
\subsubsection{Cálculo teórico de los errores estándar agrupados}

En el modelado de datos de panel, la presencia de heterocedasticidad y autocorrelación serial intra-grupo invalida la inferencia estadística basada en los errores estándares ordinarios, tendiendo a subestimar su verdadera variabilidad. Para corregir esta desviación, se implementa el estimador de matriz de covarianza robusta agrupada por clústeres (estaciones), el cual generaliza la formulación de White para permitir patrones arbitrarios de correlación entre las observaciones temporales de una misma entidad \cite{wooldridge2010}.

Considérese el modelo lineal agrupado por estaciones meteorológicas en su representación matricial para cada unidad $i$:
\begin{equation}
    y_{i} = X_{i}\beta + u_{i}, \quad i = 1, 2, \dots, M
\end{equation}
Donde $M = 18$ representa el número total de estaciones, $y_i$ es el vector de observaciones del SPEI-6 de dimensión $T_i \times 1$, $X_i$ es la matriz de regresores de dimensión $T_i \times K$, y $u_i$ es el vector de perturbaciones idiosincráticas de dimensión $T_i \times 1$. 

Bajo este esquema, se asume independencia mutua entre estaciones distintas ($Cov(u_i, u_j | X_i, X_j) = 0$ para todo $i \neq j$), pero se tolera una estructura de varianza-covarianza desconocida y no paramétrica dentro de cada clúster $i$ (denotada por $\Omega_i = E[u_i u_i' | X_i]$). El estimador robusto agrupado sustituye esta matriz por el producto cruzado de los residuos muestrales del modelo ($\hat{u}_i = y_i - X_i \hat{\beta}$).

De este modo, la estimación empírica de la matriz de varianza-covarianza de los coeficientes, $\widehat{\text{Var}}_{robust}(\hat{\beta})$, se calcula de manera compacta mediante la estructura de tipo "sándwich":
\begin{equation}
    \widehat{\text{Var}}_{robust}(\hat{\beta}) = q \cdot \left( \sum_{i=1}^{M} X_{i}' X_{i} \right)^{-1} \left( \sum_{i=1}^{M} X_{i}' \hat{u}_{i} \hat{u}_{i}' X_{i} \right) \left( \sum_{i=1}^{M} X_{i}' X_{i} \right)^{-1}
\end{equation}

Donde $q$ es el factor de corrección multiplicativo por tamaño muestral finito, diseñado para reducir el sesgo de subestimación en paneles donde el número de clústeres $M$ es limitado:
\begin{equation}
    q = \left( \frac{M}{M - 1} \right) \left( \frac{N - 1}{N - K} \right)
\end{equation}
Siendo $N = \sum T_i$ el número total de observaciones mensuales del panel ($5,094$) y $K$ el número de parámetros estimados. Los errores estándar agrupados por estación corresponden a la raíz cuadrada de los elementos de la diagonal principal de esta matriz resultante.
