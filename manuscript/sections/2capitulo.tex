...
El cálculo del índice acumula estas diferencias mensuales en diferentes escalas de tiempo históricas. Debido a que el balance hídrico puede tomar valores negativos, se recurre a la distribución Log-Logística de tres parámetros para ajustar las series temporales expresada como:
\begin{equation}
    f(x) = \frac{\beta}{\alpha} \left( \frac{x - \gamma}{\alpha} \right)^{\beta - 1} \left[ 1 + \left( \frac{x - \gamma}{\alpha} \right)^{\beta} \right]^{-2}
\end{equation}

Donde $\alpha$, $\beta$ y $\gamma$ representan los parámetros de escala, forma y localización, respectivamente, estimados mediante el método de momentos ponderados de probabilidad (L-moments). La distribución acumulada $F(x)$ se transforma posteriormente en la variable normal estandarizada $Z(\mathrm{SPEI})$ con media cero y varianza unitaria, permitiendo una clasificación objetiva de la intensidad de las anomalías hídricas (Cuadro \ref{tab:clasificacion_spei}).
